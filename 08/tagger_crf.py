...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        predictions = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(train.tags.word_mapping.vocabulary_size()))(bidirectional)
        ...
        self._crf_weights = self.add_weight('crf_weights',
            (train.tags.word_mapping.vocabulary_size(), train.tags.word_mapping.vocabulary_size()),
            dtype=tf.float32,
            trainable=True,
            initializer='zeros')
    ...
    def crf_loss(self, gold_labels: tf.RaggedTensor, logits: tf.RaggedTensor) -> tf.Tensor:
        ...
        loss, _ = tfa.text.crf_log_likelihood(logits.to_tensor(), gold_labels.to_tensor(), sequence_length, self._crf_weights)
    ...
    def crf_decode(self, logits: tf.RaggedTensor) -> tf.RaggedTensor:
        ...
        dense_predictions, _ = tfa.text.crf_decode(logits_dense, self._crf_weights, l)
        predictions = tf.RaggedTensor.from_tensor(dense_predictions, l)
