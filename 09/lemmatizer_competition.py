...
#parser.add_argument("--max_sentences", default=100, type=int, help="Maximum number of sentences to load.")
...
class Model(tf.keras.Model):
    def __init__(self, args, train):
        ...
        self.target_mapping_inverse = type(self.target_mapping)(mask_token=None, vocabulary=self.target_mapping.get_vocabulary(), invert=True)
    ...
    def call(self, inputs, targets=None):
        ...
        embed = self.source_embedding(source_charseqs)
        ...
        source_states = self.source_rnn(embed.to_tensor(), mask=tf.sequence_mask(embed.row_lengths()))
        ...
        if targets is not None:
            ...
            output, _, output_lens = DecoderTraining([source_states, target_charseqs])
        else:
            ...
            DecoderPrediction = self.DecoderPrediction(self, maximum_iterations=tf.cast(source_charseqs.bounding_shape(1) + 10, dtype=tf.int32))
            output, _, output_lens = DecoderPrediction(source_states)
            output_lens -= 1
...
def main(args):
    ...
    #def lemma_dataset(forms, lemmas, tags):
     #   return forms, lemmas

    #train = morpho.train.dataset.map(lemma_dataset).apply(tf.data.experimental.dense_to_ragged_batch(args.batch_size))
    #dev = morpho.dev.dataset.map(lemma_dataset).apply(tf.data.experimental.dense_to_ragged_batch(args.batch_size))
    #test = morpho.test.dataset.map(lemma_dataset).apply(tf.data.experimental.dense_to_ragged_batch(args.batch_size))
