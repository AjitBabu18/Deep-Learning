...
class VAE(tf.keras.Model):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        for units_count in args.encoder_layers:
            x = tf.keras.layers.Dense(units_count, activation=tf.keras.activations.relu)(x)
        z_mean = tf.keras.layers.Dense(args.z_dim)(x)
        z_sd = tf.keras.activations.exponential(tf.keras.layers.Dense(args.z_dim)(x))
        #z_sd = tf.keras.activations.exponential(z_log_variance)
        

        self.encoder = tf.keras.Model(inputs=input, outputs=[z_mean, z_sd])
        ...
        x = tf.keras.layers.Flatten()(input)
        for units_count in args.decoder_layers:
            x = tf.keras.layers.Dense(units_count, activation=tf.keras.activations.relu)(x)
        output = tf.keras.layers.Dense(MNIST.H * MNIST.W * MNIST.C, activation=tf.keras.activations.sigmoid)(x)
        output = tf.keras.layers.Reshape([MNIST.H, MNIST.W, MNIST.C])(output)

        self.decoder = tf.keras.Model(inputs=input, outputs=output)
    ...
    def train_step(self, images: tf.Tensor) -> Dict[str, tf.Tensor]:
        with tf.GradientTape() as tape:
            ...
            distribution = tfp.distributions.Normal(loc=z_mean, scale=z_sd)
            assert distribution.reparameterization_type == tfp.distributions.FULLY_REPARAMETERIZED
            z = distribution.sample(seed=self._seed)
            decoded_images = self.decoder(z, training=True)
            reconstruction_loss = self.compiled_loss(images, decoded_images, regularization_losses=self.losses)
            ...
            latent_loss = tf.math.reduce_mean(tfp.distributions.kl_divergence(distribution, tfp.distributions.Normal(0, 1)))
