...
class Agent:
    def __init__(self, env, args):
        ...
        hidden = tf.keras.layers.Conv2D(16, kernel_size= 3, strides=3, padding = 'same')(inputs)
        hidden = tf.keras.layers.BatchNormalization()(hidden)
        hidden = tf.keras.layers.ReLU()(hidden)
        #hidden = tf.keras.layers.BatchNormalization()(hidden)
        hidden = tf.keras.layers.Conv2D(32, kernel_size= 3, strides=3, padding = 'same')(hidden)
        hidden = tf.keras.layers.BatchNormalization()(hidden)
        hidden = tf.keras.layers.ReLU()(hidden)
        #hidden = tf.keras.layers.BatchNormalization()(hidden)
        ...
        hidden_base = tf.keras.layers.Conv2D(16, kernel_size= 3, strides=3, padding = 'same')(inputs)
        hidde_base = tf.keras.layers.BatchNormalization()(hidden_base)
        hidden_base = tf.keras.layers.ReLU()(hidden_base)
        hidden_base = tf.keras.layers.Conv2D(32, kernel_size= 3, strides=3, padding = 'same')(hidden_base)
        hidde_base = tf.keras.layers.BatchNormalization()(hidden_base)
        hidden_base = tf.keras.layers.ReLU()(hidden_base)
