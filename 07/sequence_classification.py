...
parser.add_argument("--clip_gradient", default=0.0003, type=float, help="Norm for gradient clipping.")
...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.compile(
            optimizer=tf.keras.optimizers.Adam(clipnorm=args.clip_gradient),
            loss=tf.losses.BinaryCrossentropy(),
            metrics=[tf.metrics.BinaryAccuracy("accuracy")],
        )

        self.tb_callback = tf.keras.callbacks.TensorBoard(args.logdir, profile_batch=0)
