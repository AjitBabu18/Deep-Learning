...
class Model(tf.Module):
    ...
    def predict(self, inputs):
        ...
        #print(self._b1, self._b2, self._W1, self._W2)
        input = tf.reshape(inputs, [inputs.shape[0], -1])
        multiply = tf.nn.tanh(tf.math.add(tf.linalg.matmul(input, self._W1), self._b1))
        result = tf.nn.softmax(tf.math.add(tf.linalg.matmul(multiply, self._W2), self._b2))
        return result
    ...
    def train_epoch(self, dataset):
        for batch in dataset.batches(self._args.batch_size):
            ...
            with tf.GradientTape() as tape:
                ...
                hotlabel = tf.one_hot(batch["labels"], MNIST.LABELS)
                x = tf.math.multiply(hotlabel, probabilities)
                loss = tf.reduce_mean(-tf.math.log(tf.math.reduce_sum(x, axis=1)))
            ...
            for variable, gradient in zip(variables, gradients):
                ...
                variable = tf.compat.v1.assign_sub(variable, self._args.learning_rate * gradient)
