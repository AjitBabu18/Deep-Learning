...
def main(args: argparse.Namespace):
    ...
    data = tf.reshape(data, (-1, data.shape[1] * data.shape[2] * data.shape[3]))
    #print(data)
    ...
    mul = tf.transpose(data - mean) @ (data - mean)
    cov = mul / data.shape[0]
    ...
    total_variance = tf.math.reduce_sum(diag)
