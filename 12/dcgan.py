...
class GAN(tf.keras.Model):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        inputs_gen = tf.keras.layers.Input([args.z_dim])

        hidden = tf.keras.layers.Dense(1024, use_bias=False)(inputs_gen)
        ...
        hidden = tf.keras.layers.Conv2DTranspose(filters=32, kernel_size=4, strides=2, padding='same', use_bias=False)(hidden)
        hidden = tf.keras.layers.BatchNormalization()(hidden)
        hidden = tf.keras.layers.Activation(tf.nn.relu)(hidden)

        output_gen = tf.keras.layers.Conv2DTranspose(filters=1, kernel_size=4, strides=2, padding='same', activation=tf.nn.sigmoid)(hidden)

        self.generator = tf.keras.Model(inputs=inputs_gen, outputs=output_gen)
        ...
        inputs_dis = tf.keras.layers.Input([MNIST.H, MNIST.W, MNIST.C])

        hidden = tf.keras.layers.Conv2D(filters=32, kernel_size=5, padding='same', use_bias=False)(inputs_dis)
        hidden = tf.keras.layers.BatchNormalization()(hidden)
        hidden = tf.keras.layers.Activation(tf.nn.relu)(hidden)
        ...
        output_dis = tf.keras.layers.Dense(1, activation=tf.nn.sigmoid)(hidden)

        self.discriminator = tf.keras.Model(inputs=inputs_dis, outputs=output_dis)
