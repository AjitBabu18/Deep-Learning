...
parser.add_argument("--classes", default=2, type=int, help="Number of classes per episode.")
...
parser.add_argument("--test_episodes", default=160, type=int, help="Number of testing episodes.")
parser.add_argument("--threads", default=1, type=int, help="Maximum number of threads to use.")
parser.add_argument("--train_episodes", default=160, type=int, help="Number of training episodes.")
...
class Model(tf.keras.Model):
    ...
    class MemoryAugmentedLSTM(tf.keras.layers.AbstractRNNCell):
        ...
        def state_size(self) -> List[tf.TensorShape]:
            ...
            return [self._controller.state_size, self._cell_size * self._read_heads, [self._memory_cells, self._cell_size]]
        ...
        def call(self, inputs: tf.Tensor, states: List[tf.Tensor]) -> Tuple[tf.Tensor, List[tf.Tensor]]:
            ...
            controller_output, controller_state = self._controller(tf.concat([inputs, read_value], axis=-1), controller_state)
            ...
            external_memory_parameters = self._parameters(controller_output)
            write_value = external_memory_parameters[:, :self._cell_size]
            read_keys = tf.reshape(external_memory_parameters[:, self._cell_size:], [-1, self._read_heads, self._cell_size])
            ...
            self_attention = tf.linalg.matmul(read_keys_normalized, memory_normalized, transpose_b=True)
            self_attention_softmax = tf.nn.softmax(self_attention, axis=-1)
            weighted_sum = tf.linalg.matmul(self_attention_softmax, memory)
            ...
            write_value = tf.expand_dims(write_value, 1)
            memory = tf.concat([write_value, memory[:, :-1, :]], axis=1)
            ...
            output = tf.concat([controller_output, read_value], axis=-1)
        ...
        def get_initial_state(self, inputs=None, batch_size=None, dtype=None):
            [_, memory_values, external_memory] = self.state_size
            init_cont_state = self._controller.get_initial_state(inputs, batch_size, dtype)
            init_mem_state = tf.zeros([batch_size, memory_values], dtype=tf.float32, name="IMSz")
            init_ext_mem_state = tf.zeros([batch_size, external_memory[0], external_memory[1]], dtype=tf.float32, name="IEMSz")
            return init_cont_state, init_mem_state, init_ext_mem_state
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        for filter in [8, 16, 32]:
            data = tf.keras.layers.Conv2D(filter, 3, 2, 'valid')(data)
            data = tf.keras.layers.BatchNormalization()(data)
            data = tf.keras.layers.ReLU()(data)
        data = tf.keras.layers.Reshape([-1, 128])(data)
        ...
        previous_labels_oh = tf.one_hot(previous_labels, args.classes)
        processed_input = tf.concat([data, previous_labels_oh], axis=-1)
        ...
        LSTM_cell = self.MemoryAugmentedLSTM(args.lstm_dim, args.classes * args.images_per_class, args.cell_size, args.read_heads)
        outputs = tf.keras.layers.RNN(LSTM_cell, return_sequences=True)(processed_input)
        ...
        predictions = tf.keras.layers.Dense(args.classes, tf.nn.softmax)(outputs)
