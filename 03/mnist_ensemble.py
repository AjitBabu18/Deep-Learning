...
def main(args: argparse.Namespace) -> Tuple[List[float], List[float]]:
    ...
    individual_accuracies, ensemble_accuracies, list_of_outputs = [], [], []
    for model in range(args.models):
        # TODO: Compute the accuracy on the test set for
        # the individual `models[model]`.
        individual_accuracy = models[model].evaluate(mnist.test.data["images"], mnist.test.data["labels"])[1]
        ...
        #ensemble_prediction.append(models[0].predict(mnist.test.data["images"]))
        
        predict=models[model].predict(mnist.test.data["images"])
        list_of_outputs.append(predict)
        avg_output = tf.math.reduce_mean(list_of_outputs, axis=0)
        accuracy= tf.metrics.SparseCategoricalAccuracy()
        accuracy.update_state(mnist.test.data["labels"],avg_output)
