...
def main(args: argparse.Namespace) -> float:
    ...
    if args.l2 != 0:
        L2_regularization = tf.keras.regularizers.L2(l2=args.l2)
    ...
    for hidden_layer in args.hidden_layers:
        model.add(tf.keras.layers.Dense(hidden_layer, activation=tf.nn.relu, kernel_regularizer=L2_regularization))
        if Dropout != None:
            model.add(tf.keras.layers.Dropout(Dropout))
    model.add(tf.keras.layers.Dense(MNIST.LABELS, activation=tf.nn.softmax,kernel_regularizer=L2_regularization))
    ...
    _loss= tf.keras.losses.CategoricalCrossentropy(label_smoothing=args.label_smoothing)
    _metric= tf.metrics.CategoricalAccuracy(name="accuracy")
